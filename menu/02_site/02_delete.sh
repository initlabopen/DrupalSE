...
delete_site() {
    ...
    if [[ ! -d "$site_dir"  ]]; then
        output_text "Press ENTER and exit" \
            "Directory $site_dir doesn't exist in the system" any_key
        exit 1
    fi
    ...
    if [[ ( $is_site -eq 0 ) ]]; then
        output_text "Press ENTER and exit" \
               "Not found site directory $site_dir" any_key
        exit
    ...
    any_key=
    if [[ -n "$error" ]]; then
        output_text "DELETE_SITE error: Press ENTER for exit: " "$error"  any_key
    else
        output_text "DELETE_SITE complete: Press ENTER for exit: " ""  any_key
    fi
...
_menu_delete() {
  ...
  until [[ -n "$SITE_MENU_SELECT" ]]; do
    ...
    output_text 'Enter site directory (ex. /home/webmaster/domains/example.com) or 0 for exit: ' ""  SITE_MENU_SELECT
